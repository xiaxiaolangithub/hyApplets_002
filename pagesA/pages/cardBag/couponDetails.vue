<template>
	<view class="couponDetailsPage">
		...
		<view class="coupon_footer" :class="couponObj.type==='DEDUCTION'?'':'special_footer'" v-if="road==='center'"  hover-class="handle_hover" :hover-stay-time="150" @tap="toUser(2)">
			<view>立即领取</view>
		</view>
	</view>
	...
	export default {
		...
		methods:{
			...
			transData(result) {
				...
				if(this.type !== 2) {
					this.couponData[0].backImg = result.type === 'DEDUCTION' ? this.realmImg('cardBag/new_back.png') : this.realmImg('cardBag/soon_back.png');
				} else {
				...
				if(this.road === 'center') {
					this.couponObj.getBeginTime = `${result.getBeginTime.replace(/-/g, '.')}-${result.getEndTime.replace(/-/g, '.')} `
					this.couponObj.activeDate = result.duration !== 0 ? `领取${result.activeInterval}天后生效，有效期${result.duration}天` : `${result.activeTime.replace(/-/g, '.')}-${result.failureTime.replace(/-/g, '.')} `
				} else {
					this.couponObj.activeDate = `${result.activeTime.replace(/-/g, '.')}-${result.failureTime.replace(/-/g, '.')} `
