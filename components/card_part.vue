<template>
	<!-- 当前这个组件为卡包及券详情的组件 -->
	<view>
		<view class="list" v-for="(list, index) in couponData" :key="index" @tap="toDetail(list)">
			<img class="back_img" :src="list.backImg"></img>
			<img class="tip_img" :src="realmImg('cardBag/new_line.png')" v-if="list.isNew==='YES'&&navIndex==0"></img>
			
			<!-- 即将到期都要有标注即将到期 -->
			<img class="tip_img" :src="realmImg('cardBag/soon_line.png')" v-if="navIndex===1"></img>
			<view class="left">
				<!-- 券换券并且礼品金额为0的时候显示1张 -->
				<view class="price" style="margin-bottom: 12px;" v-if="list.type==='GIFT'&&list.amount===0">1张</view>
				<view class="price" v-else>
					<text class="price_icon">¥</text>
					{{list.amount || 0}}
				</view>
				<view class="type">{{list.type === 'DEDUCTION' ? '代金券' : '兑换券'}}</view>
			</view>
			<view class="middle">
				<!-- 区分代金券、兑换券、失效券3种情况券名称的样式 -->
				<view class="name" v-if="navIndex!==2" :class="list.type==='DEDUCTION'?'newName':'soonName'">{{list.voucherName}}</view>
			...
			<view class="right">
				<!-- @click.stop="toUser" -->
				<view class="btn" hover-class="handle_hover" :class="list.type==='DEDUCTION'?'newBtn':'soonBtn'" v-if="navIndex!==2&&road==='own'" >去使用</view>
				<view class="btn" hover-class="handle_hover" :class="list.type==='DEDUCTION'?'newBtn':'soonBtn'" v-if="road==='center'" @click.stop="receiveCoupon(list)">立即领取</view>
...
<script>
	import { receiveCoupon, goTaBar } from '@/utils/request.js'
	export default {
		props: {
			couponData: {		// 券对象
				type: Array,
