<template>
	<view>
		<tui-bottom-popup :show="isPopup" class="coupons_popup">
			<view class="long-data-changeTime long-data-fr">
				{{year}}年{{month}}月{{day}}日 {{week}} {{hour}}:{{minute}}{{ type == 'day' ? '(' + desc + ')' : ''}}
			</view>
			...
			<button v-if="showCheck" type="warn" class="btn">{{checkStr}}</button>
			<button v-else type="primary" class="btn save" @tap="closePop">确定</button>
		</tui-bottom-popup>
	</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			//初始化时间
			initDate() {
				this.dateObj.hours = ["09", "10", "11", "12", "13", "14", "15", "16", "17", "18"];
				this.dateObj.minutes = ["00", "10", "20", "30", "40", "50", "00"];
			},
			//滚动切换时间
			bindDateChange(e) { //有效日期的滚动日期时间方法
				...
				let chooes_time = new Date(dateStr.replace(/-/g, "/") + ' ' + hourStr + ':' + minuteStr)
				this.chooesDate = chooes_time
				this.year = chooes_time.getFullYear()
				this.month = chooes_time.getMonth() + 1  < 10 ? '0' + chooes_time.getMonth() + 1 : chooes_time.getMonth() + 1
				this.day = chooes_time.getDate() < 10 ? '0' + chooes_time.getDate() : chooes_time.getDate()
